...
export interface DBTask {
  ...
  reporter_note?: string; // Ghi chú từ người báo cáo (Nhân viên)
  start_time: string; 
  end_time: string;   
...
// --- 2. MOCK DATA CẬP NHẬT THÊM REPORTER NOTE ---
  ...
  tasks: [
    {
      ...
      reporter_note: 'Em đã tối ưu lại CSS cho mobile, phần nhịp thở đang căn chỉnh thêm.',
    ...
    },
    {
      ...
      reporter_note: 'Đã hoàn thành thu thập dữ liệu từ 5 đối thủ chính.',
            ...
            return (
              <div key={task.id} className="relative flex items-start gap-6 md:gap-10 group">
                ...
                `}>
                  ...
                  {/* KHU VỰC COMMENT (PHẢN HỒI) */}
                  <div className="space-y-3 mb-6">
                    {/* 1. Comment của Người báo cáo (Nhân viên) */}
                    {task.reporter_note && (
                      <div className="p-4 rounded-2xl bg-foreground/[0.03] border border-border/50 flex gap-3 items-start animate-in fade-in slide-in-from-left-2">
                        <User size={14} className="text-muted-foreground/60 mt-1 shrink-0" />
                        <div>
                          <p className="text-[9px] font-black text-muted-foreground/60 uppercase tracking-widest mb-1">Reporter's Note</p>
                    ...
                    {MOCK_DB_REPORT.manager_comment && !isDone && (
                      <div className="p-4 rounded-2xl bg-primary/5 border border-primary/10 flex gap-3 items-start animate-in fade-in slide-in-from-top-2">
                        <MessageSquare size={14} className="text-primary mt-1 shrink-0" />
                        <div>
                          <p className="text-[9px] font-black text-primary uppercase tracking-widest mb-1">Admin Feedback</p>
                          <p className="text-[11px] italic text-primary/80 leading-relaxed font-medium">
