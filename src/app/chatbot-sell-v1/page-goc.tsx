...
//export default 
function ChatSellPage__ChayDuoc() {
  const { messages, input, handleInputChange, handleSubmit, isLoading } = useChat({ 
    api: '/api/productchatbot/bot-sell' 
  });
  ...
  return (
    ...
    <div className="flex flex-col h-[100dvh] max-w-4xl mx-auto bg-background overflow-hidden relative">
      ...
      {/* DANH SÁCH TIN NHẮN: Là phần duy nhất được scroll */}
      <div className="flex-1 overflow-y-auto px-4 flex flex-col-reverse custom-scrollbar">
        {/* Khoảng trống để tin nhắn cuối không bị dính sát vào input */}
      ...
      {/* INPUT AREA: Cố định ở đáy bằng shrink-0 */}
      ...
      <style jsx global>{`
        /* Ẩn scrollbar chính của body để tránh scroll kép */
