    ...
    <div className="relative">
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute top-full left-0 mt-4 w-[280px] md:w-[450px] bg-card/95 backdrop-blur-2xl border border-border rounded-[2rem] p-6 shadow-[0_20px_50px_rgba(0,0,0,0.2)] animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-h-[60vh] overflow-y-auto custom-scrollbar pr-2">
              {categoryTree.map((parent: any) => (
                <div key={parent.id} className="space-y-3">
                  {/* Category Cha */}
                  <Link
                    href={`/testSearchParam/${parent.category_path}`}
                  ...
                  {parent.children && (
                    <div className="flex flex-col gap-2 pl-3 border-l border-border/60">
                      {parent.children.map((child: any) => (
                        <Link
                          key={child.id}
                          href={`/testSearchParam/${child.category_path}`}
                          onClick={() => setIsOpen(false)}
