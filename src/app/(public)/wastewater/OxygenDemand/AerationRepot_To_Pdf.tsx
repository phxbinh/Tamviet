...
export const AerotankReportView: React.FC<AerotankReportViewProps> = ({ data }) => {
  // Tạo một tham chiếu (ref) đến khối giao diện cần in bản cứng
  const reportRef = useRef<HTMLDivElement>(null);

  // Cấu hình hàm kích hoạt hộp thoại in hệ thống
  const handlePrint = useReactToPrint({
    contentRef: reportRef,
    documentTitle: `Thuyet_Minh_Thiet_Ke_Aerotank_${Date.now()}`,
  });
...
$$S = \\frac{K_s \\cdot [1 + b_H \\cdot SRT]}{SRT \\cdot (Y_H \\cdot k_rate - b_H) - 1}$$
...
`;
  ...
  const markdownComponents = useMemo(() => ({
    ...
    div: ({ className, children }: any) => {
      if (className?.includes("math-display")) {
        return (
          // Thêm thuộc tính 'print:break-inside-avoid' để không bao giờ cắt đôi công thức toán học khi ngắt trang PDF
  ...
  return (
    <div className="min-h-screen bg-neutral-50/40 py-4 sm:py-10 px-2 sm:px-6 font-sans">
      ...
      {/* Khối thanh điều hướng chứa Nút bấm Xuất PDF - Sẽ ẩn đi khi in */}
      <div className="max-w-3xl mx-auto mb-4 flex justify-end print:hidden">
        ...
        >
          <span>📄</span> Xuất báo cáo PDF / In thuyết minh
        </button>
      </div>

      {/* Gắn ref vào vùng layout cần xuất PDF */}
      <div ref={reportRef} className="max-w-3xl mx-auto bg-white border border-neutral-200/60 shadow-xs rounded-xl overflow-hidden print:border-none print:shadow-none print:rounded-none">
        ...
        {/* Style bổ sung dành riêng cho quy trình render máy in PDF */}
        ...
        <div className="px-6 py-4 bg-neutral-900 text-white flex justify-between items-center print:bg-neutral-900 print:text-white">
          <div>
            ...
            <h2 className="text-base font-light tracking-wide text-neutral-100 mt-0.5 print:text-sm">Biện Luận Kỹ Thuật Bể Sinh Học</h2>
          </div>
        ...
        <div className="mx-5 sm:mx-10 mb-8 p-4 bg-neutral-50 border-l border-neutral-900 rounded-r-md page-break-inside-avoid print:mx-4 print:mb-4 print:bg-neutral-50 print:break-inside-avoid">
          <div className="flex items-start gap-2.5">
            ...
            <div>
              <h4 className="text-[11px] font-bold text-neutral-900 uppercase tracking-wider print:text-[10px]">Khuyến nghị vận hành từ AI Agent</h4>
