...
const DynamicWastewaterDiagram: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Tìm vị trí của các bể trong chuỗi hiện tại
    ...
    // Tìm bể lắng bậc hai nằm cuối cùng bằng vòng lặp ngược (Tránh lỗi tương thích build)
    ...
    // 1. TUẦN HOÀN BÙN THÔNG MINH (RAS): Tự động chọn điểm đích (Anoxic -> Aerobic)
    if (secondaryClarifierIdx !== -1) {
      let targetIdx = -1;
      let rasLabel = '';

      // Ưu tiên 1: Tuần hoàn về bể Anoxic nếu nó đứng trước bể lắng bậc hai
      if (anoxicIdx !== -1 && secondaryClarifierIdx > anoxicIdx) {
        targetIdx = anoxicIdx;
        rasLabel = 'Return Activated Sludge (To Anoxic Tank)';
      } 
      // Ưu tiên 2: Nếu không có Anoxic, tuần hoàn về bể Aerobic (bể hiếu khí) nếu nó đứng trước bể lắng
      else if (aerationIdx !== -1 && secondaryClarifierIdx > aerationIdx) {
        targetIdx = aerationIdx;
        rasLabel = 'Return Activated Sludge (To Aerobic Tank)';
      }

      // Chỉ vẽ nếu tìm được điểm trả về thích hợp
      if (targetIdx !== -1) {
        const startRAS_X = startX + secondaryClarifierIdx * tankSpacing + tankW / 2;
        const endRAS_X = startX + targetIdx * tankSpacing - 25; // Nối vào trước bồn đích 25px

        drawPolylineArrow([
          { x: startRAS_X, y: yMain + tankH + 20 },      // Trích từ ống bùn đáy
          { x: startRAS_X, y: yMain + tankH + 60 },      // Đi xuống làn chạy ngầm dưới đáy canvas
          { x: endRAS_X, y: yMain + tankH + 60 },        // Chạy ngược dòng về bên trái
          { x: endRAS_X, y: yMain + 25 }                 // Đi thẳng đứng lên hòa vào dòng nạp của bồn đích
        ]);
        ctx.font = '11px Arial';
        ctx.fillText(rasLabel, endRAS_X + 20, yMain + tankH + 53);
      }
    }

    // 2. TUẦN HOÀN NƯỚC NỘI BỘ (Internal Recycle): Từ Aeration về Anoxic (Chạy phía TRÊN)
    if (aerationIdx !== -1 && anoxicIdx !== -1 && aerationIdx > anoxicIdx) {
      ...
      drawPolylineArrow([
        { x: startRecycleX, y: yMain },               // Hút từ bề mặt bể Aeration
        { x: startRecycleX, y: yMain - 40 },          // Đẩy vọt lên cao trên không trung
        { x: endRecycleX, y: yMain - 40 },            // Chạy ngang sang trái
        { x: endRecycleX, y: yMain }                  // Xả trực diện xuống lòng bể Anoxic
      ]);
      ctx.font = '11px Arial';
