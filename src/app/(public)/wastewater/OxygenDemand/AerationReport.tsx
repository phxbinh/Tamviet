...
export const AerotankReportView: React.FC<AerotankReportViewProps> = ({ data }) => {
  ...
  const reportMarkdown = useMemo(() => {
    ...
    [span_0](start_span)const k_rate = mu_max / y_h;[span_0](end_span)
    [span_1](start_span)const s_eff = (k_s * (1 + b_h * srt)) / (srt * (y_h * k_rate - b_h) - 1);[span_1](end_span)

    [span_2](start_span)[span_3](start_span)const term1 = (q * y_h * (s_0 - s_eff) * srt) / ((1 + b_h * srt) * 0.85 * x_tss);[span_2](end_span)[span_3](end_span)
    [span_4](start_span)[span_5](start_span)const term2 = (f_d * b_h * q * y_h * (s_0 - s_eff) * Math.pow(srt, 2)) / ((1 + b_h * srt) * 0.85 * x_tss);[span_4](end_span)[span_5](end_span)
    [span_6](start_span)const v_tank = term1 + term2;[span_6](end_span)
    [span_7](start_span)const tau_days = v_tank / q;[span_7](end_span)
    [span_8](start_span)const tau_hours = tau_days * 24;[span_8](end_span)

    [span_9](start_span)const p_x_bio = (0.85 * x_tss * v_tank) / srt;[span_9](end_span)
    [span_10](start_span)const r_o_day = q * (s_0 - s_eff) - 1.42 * p_x_bio;[span_10](end_span)
    [span_11](start_span)const aotr_kg_h = (r_o_day / 1000) / 24;[span_11](end_span)

    [span_12](start_span)const p_x_tss_kg = (x_tss * v_tank / srt) / 1000;[span_12](end_span)
    [span_13](start_span)const sbod_eff = s_eff / bcod_bod_ratio;[span_13](end_span)

    [span_14](start_span)const p_b_over_p_s = Math.exp(-(9.81 * 28.97 * elevation) / (8314 * (273.15 + temp)));[span_14](end_span)
    [span_15](start_span)const c_inf_20 = 9.09 * (10.33 + depth_sat) / 10.33;[span_15](end_span)
    [span_16](start_span)const c_s_t_over_c_inf_20 = 10.08 / 9.09;[span_16](end_span)

    [span_17](start_span)const numerator = c_inf_20;[span_17](end_span)
    [span_18](start_span)const denominator = ((beta * p_b_over_p_s * c_s_t_over_c_inf_20 * c_inf_20) - do_target) * alpha * f_factor * Math.pow(1.024, (temp - 20));[span_18](end_span)
    [span_19](start_span)const sotr_over_aotr = numerator / denominator;[span_19](end_span)
    [span_20](start_span)const sotr_kg_h = aotr_kg_h * sotr_over_aotr;[span_20](end_span)

    [span_21](start_span)const rho_air_altitude = p_b_over_p_s * 1.225;[span_21](end_span)
    [span_22](start_span)const o2_per_m3_air = 0.2318 * rho_air_altitude;[span_22](end_span)
    [span_23](start_span)const air_flowrate_m3_min = sotr_kg_h / (ote * 60 * o2_per_m3_air);[span_23](end_span)
