...
<div className="space-y-6">
  {attributes.map((attr) => {
    ...
    return (
      <div key={attr.id} className="group/attr">
        {/* Header của thuộc tính: Nhỏ gọn, sát với các nút bấm */}
        <div className="flex items-baseline justify-between mb-2 px-1">
          <label className="text-[10px] font-black uppercase tracking-[0.15em] text-muted-foreground/60 group-hover/attr:text-primary transition-colors">
            {attr.name}
          </label>
          
          {/* Phản ứng tức thì: Hiển thị giá trị đang chọn ngay cạnh nhãn */}
          {currentValue && (
            <span className="text-[10px] font-bold text-primary animate-in fade-in slide-in-from-right-2 duration-300">
              {currentValue}
            </span>
          )}
        </div>

        {/* Nút bấm: Sát nhau, giảm padding thừa, phản ứng click mạnh */}
          ...
          {attr.values.map((v) => {
            ...
            return (
              <button
                ...
                className={`
                  relative px-4 py-2 text-[11px] font-bold uppercase tracking-wider transition-all duration-200 rounded-lg border
                  ${active 
                    ? "bg-foreground text-background border-foreground shadow-md scale-[1.02] z-10" 
                    : "bg-background border-border text-foreground/70 hover:border-primary/50 hover:bg-secondary/30 active:scale-95"
                  }
                `}
              >
                {v.value}
                {/* Chấm tròn nhỏ chỉ báo khi active để tạo điểm nhấn tinh tế */}
                {active && (
...
{/* 3. Conversion Hub (Gần gũi hơn với lựa chọn cuối cùng) */}
  ...
  <div className="min-h-[80px] flex flex-col justify-end">
    {selectedVariant ? (
      <div className="animate-in fade-in zoom-in-95 duration-300 space-y-4">
        {/* Thông tin kho và SKU hiển thị sát nút bấm */}
        <div className="flex items-center gap-4 bg-secondary/20 p-3 rounded-xl border border-border/40">
           <div className="flex-1">
              <p className="text-[8px] font-black opacity-40 uppercase">Inventory Status</p>
              <p className={`text-[11px] font-black ${selectedVariant.stock < 10 ? 'text-orange-500' : 'text-foreground'}`}>
                {selectedVariant.stock < 10 ? `Low Stock: ${selectedVariant.stock} units` : 'In Stock & Ready'}
              </p>
           </div>
           <div className="h-8 w-[1px] bg-border" />
           <div className="flex-1 text-right">
              <p className="text-[8px] font-black opacity-40 uppercase">Ref.</p>
              <p className="text-[10px] font-mono font-bold tracking-tighter text-muted-foreground">{selectedVariant.sku}</p>
           </div>
        </div>
        ...
        <button
          ...
          className="w-full py-4 bg-primary text-primary-foreground rounded-2xl font-black text-[12px] uppercase tracking-[0.3em] shadow-lg shadow-primary/20 hover:shadow-primary/40 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
        >
          {isAdding ? (
            <span className="flex items-center gap-2 italic animate-pulse">Adding to vault...</span>
          ) : (
      ...
      <div className="py-8 text-center border-2 border-dashed border-border rounded-2xl opacity-40">
        <p className="text-[10px] font-black uppercase tracking-[0.2em]">Select variants to unlock</p>
      </div>
    )}
  </div>
</div>
