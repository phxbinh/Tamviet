...
//export 
async function getProductDetail_slug__(slug: string): Promise<ProductFull | null> {
  try {
...
`;
    ...
    for (const row of variantRows) {
      ...
      if (row.attribute_name) {
        variantsMap.get(row.variant_id).attributes[row.attribute_name] = row.attribute_value;
      }
    }
    ...
    return { product, attributes, variants, images };
