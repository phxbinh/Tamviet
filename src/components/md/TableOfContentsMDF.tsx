...
export default function TableOfContents({ htmlContent, contentRef }: TocProps) {
  ...
  // Flag dùng để chặn Highlight nhảy loạn khi đang đóng/mở menu
  const [isLock, setIsLock] = useState(false);

  // 1. Trích xuất Heading và chuẩn hóa ID
  const toc = useMemo(() => {
    if (typeof window === 'undefined' || !htmlContent) return [];
    const parser = new DOMParser();
    const doc = parser.parseFromString(htmlContent, 'text/html');
  ...
  // 2. Xử lý đóng mở menu có "Khóa"
  const handleToggle = (e: React.SyntheticEvent<HTMLDetailsElement>) => {
    const newState = e.currentTarget.open;
    if (newState !== isOpen) {
      setIsLock(true); // Khóa observer lại
      setIsOpen(newState);
      // Đợi hiệu ứng trượt xong (approx 400ms) mới mở khóa
      setTimeout(() => setIsLock(false), 400);
    }
  };

  useEffect(() => {
    // Gán ID vào DOM thực tế của bài viết
    toc.forEach((item, index) => {
      if (!contentRef.current) return;
    ...
    const handleScroll = () => {
      if (!contentRef.current || isLock) return;
      const element = contentRef.current;
      const rect = element.getBoundingClientRect();
      const elementHeight = element.offsetHeight;
      const scrolled = -rect.top;
      ...
      const progress = (scrolled / (elementHeight - viewportHeight)) * 100;
      setReadProgress(Math.max(0, Math.min(100, progress)));
    };
    ...
    return () => window.removeEventListener('scroll', handleScroll);
  }, [toc, contentRef, isLock]);

  // 3. Scroll Spy (Highlight mục đang đọc) - Đã tối ưu cho Mobile
  useEffect(() => {
    if (!contentRef.current || isLock) return;
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          // Chỉ kích hoạt khi mục nằm ở vùng 20% - 40% phía trên màn hình
          if (entry.isIntersecting) {
            setActiveId(entry.target.id);
          }
        });
      ...
      { 
        // Vùng quét cực nhạy: tập trung vào phần trên của viewport
  ...
  return (
    <div className="w-full my-4 relative">
      <details 
        ref={detailsRef}
        className="group w-full bg-card/60 backdrop-blur-xl border border-border rounded-2xl overflow-hidden shadow-2xl transition-all duration-300"
        open={isOpen}
      ...
      >
        <summary className="list-none cursor-pointer p-4 flex items-center justify-between select-none hover:bg-neon-cyan/5 focus:outline-none appearance-none [&::-webkit-details-marker]:hidden">
          <div className="flex items-center gap-4">
            <div className="relative p-2 rounded-xl bg-neon-cyan/10 text-neon-cyan border border-neon-cyan/20 shadow-[0_0_20px_rgba(6,182,212,0.15)]">
              <ListTree size={20} />
            </div>
            <div className="flex flex-col">
              <span className="text-[11px] font-black uppercase tracking-[0.2em] text-foreground/80 leading-none">Mục lục</span>
              <div className="flex items-center gap-2 mt-1.5">
                <div className="w-20 h-1 bg-border/30 rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-neon-cyan shadow-[0_0_10px_#06b6d4] transition-all duration-500" 
                    style={{ width: `${readProgress}%` }} 
                  />
                </div>
                <span className="text-neon-cyan text-[10px] font-bold font-mono">{Math.round(readProgress)}%</span>
              </div>
            </div>
          </div>
        ...
        <nav className="px-4 pb-6 pt-2 border-t border-border/30 bg-black/5 animate-in fade-in slide-in-from-top-2 duration-300">
          <ul className="space-y-1 relative list-none m-0 p-0">
...
{/* Close progress bar vertical on left side of menu
            ...
            */}
            {toc.map((item, idx) => {
              const isActive = activeId === item.id;
              return (
                <li key={`${item.id}-${idx}`} style={{ paddingLeft: `${(item.level - 2) * 20}px` }} className="relative z-10 pt-1">
                  <a 
                    href={`#${item.id}`} 
                    onClick={() => { if (window.innerWidth < 1024) setIsOpen(false); }}
                    className={`
                      group/item flex items-center gap-4 py-2 px-4 text-[13px] transition-all duration-300 rounded-xl relative
                      ${isActive ? 'text-neon-cyan font-bold translate-x-1' : 'text-foreground/50 hover:text-foreground/80 hover:bg-white/5'}
                    `}
                  >
                    {/* Dot indicator */}
                    <div className="relative flex items-center justify-center">
                      {isActive && <span className="absolute w-4 h-4 bg-neon-cyan/20 animate-ping rounded-full" />}
                      <span className={`
                        w-2.5 h-2.5 rounded-full border-2 transition-all duration-500 z-20 bg-background
                        ${isActive ? 'border-neon-cyan scale-110 shadow-[0_0_10px_#06b6d4]' : 'border-border'}
                      `} />
                    </div>
