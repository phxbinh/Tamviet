...
export function Toast() {
  ...
  const configs = {
    success: { icon: CheckCircle2, color: "text-neon-cyan", border: "border-neon-cyan/50", bg: "bg-neon-cyan/10", shadow: "shadow-[0_0_20px_rgba(34,211,238,0.2)]" },
    error: { icon: AlertCircle, color: "text-red-500", border: "border-red-500/50", bg: "bg-red-500/10", shadow: "shadow-[0_0_20px_rgba(239,68,68,0.2)]" },
    info: { icon: Info, color: "text-neon-purple", border: "border-neon-purple/50", bg: "bg-neon-purple/10", shadow: "shadow-[0_0_20px_rgba(168,85,247,0.2)]" },
  };
      ...
      {toasts.map((toast) => {
        ...
        return (
          ...
          >
            <div className={`relative flex items-center gap-4 px-5 py-4 rounded-2xl bg-card/80 backdrop-blur-xl border ${config.border} ${config.shadow} min-w-[300px] max-w-sm`}>
              <div className={`absolute inset-0 -z-10 rounded-2xl ${config.bg}`} />
              <div className={`${config.color}`}><Icon size={24} strokeWidth={2.5} /></div>
              <div className="flex-1">
                <p className="text-sm font-semibold text-foreground tracking-tight leading-snug">{toast.message}</p>
              </div>
              <button onClick={() => removeToast(toast.id)} className="p-1 rounded-lg hover:bg-foreground/10 text-muted-foreground transition-all">
                <X size={16} />
              </button>
            </div>
          </div>
